...
def test_create_library(db):
    library_data = {"name": "Test Library", "metadata": {"description": "A test library"}}
    db_library = crud_library.create(db, obj_in=library_data)
    assert db_library.name == "Test Library"
    assert db_library.metadata == {"description": "A test library"}
...
def test_update_library(db):
    ...
    assert updated_library.metadata == {"description": "A test library"}
